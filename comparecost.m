function comparecost(CONSTMETHOD)
%{
...
BACH = 10000; 
DIM = deg2rad(7.0124);
LLIM =DIM - 0.035 ;% 0.122-0.035 (rad)
ULIM =DIM + 0.035 ;% = 0.122+0.035 (rad)
...
METRIC = 1;
CONST = initCONST(BACH,PRICE,DIM,LLIM,ULIM,STEP,TAGUCH_K,0,CONSTMETHOD,REWORK,INSPECT,METRIC);
...
init_sigma1 = optsigma(1);
tol1 = optk(1) * init_sigma1;%The tolerance is a constant times sigma
...
init_sigma2 = optsigma(2);
tol2 = optk(2) * init_sigma2;
...
init_sigma3 = optsigma(3);
tol3 = optk(3) * init_sigma3;
part3 = partrate(a,b,e,f,part3_dim,[0,0],init_sigma3,tol3,init_processIndex3);
...
switch CONST.METRIC
    case 0 % if benefit is used as the metric
        ...
        [maxProfit,~,~] = computeTotalProfit(allParts,part1,0,0,CONST);
        metric = maxProfit;
    case 1 % if unit is used as the metric
        [unitCost,~,~] = computeUnitCost(allParts, part1,0, 0, CONST); 
